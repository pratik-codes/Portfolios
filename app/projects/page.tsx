...
export default function Projects() {
  ...
  useEffect(() => {
    const description = "Welcome to my projects page. Here you'll find a curated list of my professional and personal endeavors."
    let i = 0
              ...
              <table className="w-full">
                <tbody>
                  {PortfolioData.ProfessionalExperience.map((project, index) => (
                    <tr key={index} className={index !== PortfolioData.ProfessionalExperience.length - 1 ? 'border-b border-green-500' : ''}>
                      <td className="p-2 border-r border-green-500 align-top">
                        <Terminal size={16} />
                      </td>
                      <td className="p-2">
                        <Link href={project.link} target="_blank" className="block hover:bg-green-900/20">
                          ...
                          <div className="mt-1">
                            {project.stack.map(tag => (
                              <span key={tag} className="text-xs mr-2">#{tag}</span>
                            ))}
                          </div>
                        </Link>
                      </td>
              ...
              <table className="w-full">
                <tbody>
                  {PortfolioData.PersonalProjects.map((project, index) => (
                    <tr key={index} className={index !== PortfolioData.PersonalProjects.length - 1 ? 'border-b border-green-500' : ''}>
                      <td className="p-2 border-r border-green-500 align-top">
                        <Terminal size={16} />
                      </td>
                      <td className="p-2">
